...
//
double
MEAMBO::G_gam(const double gamma, const int ibar, int& errorflag) const
{
...
//
double
MEAMBO::dG_gam(const double gamma, const int ibar, double& dG) const
{
  double gsmooth_switchpoint;
  double G;
...
//
double
MEAMBO::zbl(const double r, const int z1, const int z2)
{
  int i;
...
//
double
MEAMBO::erose(const double r, const double re, const double alpha, const double Ec, const double repuls,
...
//
void
MEAMBO::get_shpfcn(const lattice_t latt, const double sthe, const double cthe, double (&s)[3])
{
  switch (latt) {
    case FCC:
    case BCC:
    ...
    case HCP:
      ...
      s[2] = 1.0 / 3.0;
      break;
    case CH4: // CH4 actually needs two shape factors for diamond for C and dimer for H      
    case DIA:
    case DIA3:
      s[0] = 0.0;
      s[1] = 0.0;
...
//
int
MEAMBO::get_Zij(const lattice_t latt)
{
  switch (latt) {
    case FCC:
      return 12;
...
//
int
MEAMBO::get_Zij2(const lattice_t latt, const double cmin, const double cmax, 
                const double stheta, double& a, double& S)
{
  ...
  case DIA3: // 3NN
    ...
    numscr = 4;
    if (cmin < 0.500001) {
        //          call error('can not do 3NN MEAM for dia')
    }
    break;
...
int
MEAMBO::get_Zij2_b2nn(const lattice_t latt, const double cmin, const double cmax, double &S)
{
