...
void
MEAMBO::meam_setup_done(double* cutmax)
{
  int nv2, nv3, m, n, p;

  //     Force cutoff
  this->rcut = this->rc_meam;
        ...
        if (this->nn2_meam[a][b] == 1) {
          ...
          if (this->lattce_meam[a][b] == B1 || this->lattce_meam[a][b] == B2 ||
            ...
            if (this->lattce_meam[a][b] == B1 || this->lattce_meam[a][b] == B2 ||
              ...
              S22 = MathSpecial::powint(s221, 4);
              this->phir[nv2][j] = this->phir[nv2][j] - 0.75 * S11 * phiaa - 0.25 * S22 * phibb;
            }

          } else {
            // To avoid nan values of phir due to rapid decrease of valuse of 
            // b2nn^n and phi_meam.
            if (scrn > 0.0) {            
              nmax = 10;            
              b2nn = -Z2*scrn/Z1;
...
//
double
MEAMBO::phi_meam(double r, int a, int b)
{
  double G1, G2, s1[3], s2[3];
  double Gam1, Gam2, Z1, Z2;
  double rhobar1, rhobar2, F1, F2;
  double rho11, rho21, rho31;
  double rho12, rho22, rho32;
  ...
  if (this->lattce_meam[a][b] == C11) {
    ...
    if (scrn > 0.0) {
      nmax = 10;      
      for (n = 1; n <= nmax; n++) {
        phiaa += MathSpecial::powint((-Z2nn * scrn / Z1nn), n)
                * phi_meam(r * MathSpecial::powint(arat, n), a, a);
      }
    }
...
void
MEAMBO::compute_reference_density(void)
{
...
void
MEAMBO::get_tavref(double* t11av, double* t21av, double* t31av, double* t12av, double* t22av, double* t32av,
                 ...
                 int b, lattice_t latt)
{
  ...
  if (this->ialloy == 2) {
    ...
    *t22av = t22;
    *t32av = t32;
  } else if (latt == FCC || latt == BCC || latt == DIA || latt == DIA3 || latt == HCP 
              || latt == B1 || latt == DIM || latt == B2 || latt == CH4 || latt==LIN || latt==ZIG || latt==TRI) {
...
//-----------------------------------------------------------------------
void
MEAMBO::get_sijk(double C, int i, int j, int k, double* sijk)
{
  double x;
...
void
MEAMBO::get_densref(double r, int a, int b, double* rho01, double* rho11, double* rho21, double* rho31,
...
void
MEAMBO::interpolate_meam(int ind)
{
  int j;
  double drar;
